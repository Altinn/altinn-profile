...
public interface IChangeLogClient
{
    ...
    /// </summary>
    /// <param name="changeId">The change id to start from (exclusive or inclusive per API contract).</param>
    ...
    Task<ChangeLog?> GetChangeLog(int changeId, DataType dataType, CancellationToken cancellationToken);
}
