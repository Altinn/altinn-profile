...
namespace Altinn.Profile.Core.ProfessionalNotificationAddresses
{
        ...
        IAddressVerificationRepository addressVerificationRepository) : IProfessionalNotificationsService
    {
        ...
        private async Task<(VerificationType? EmailVerificationStatus, VerificationType? SmsVerificationStatus)> GetVerificationStatusAsync(UserPartyContactInfo notificationAddress, CancellationToken cancellationToken)
        {
            ...
            {
                emailVerificationStatus = await _addressVerificationRepository.GetVerificationStatusAsync(notificationAddress.UserId, AddressType.Email, notificationAddress.EmailAddress, cancellationToken);
            }

            if (!string.IsNullOrWhiteSpace(notificationAddress.PhoneNumber))
            {
                smsVerificationStatus = await _addressVerificationRepository.GetVerificationStatusAsync(notificationAddress.UserId, AddressType.Sms, notificationAddress.PhoneNumber, cancellationToken);
            }
