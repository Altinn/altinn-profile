...
namespace Altinn.Profile.Tests.IntegrationTests.API.Controllers
{
        ...
        : IClassFixture<ProfileWebApplicationFactory<Program>>
    {
        ...
        [Fact]
        public async Task GetAllNotificationAddresses_WhenRepositoryReturnsValues_IsOk()
        {
            // Arrange
            const int UserId = 2516356;
            var partyGuid1 = Guid.NewGuid();
            var partyGuid2 = Guid.NewGuid();
            ...
            var contactInfos = new List<UserPartyContactInfo>
            {
                ...
                {
                    UserId = UserId,
                    PartyUuid = partyGuid1,
                    EmailAddress = "inbox@me.com",
                    PhoneNumber = "+4798765432",
                    UserPartyContactInfoResources = new List<UserPartyContactInfoResource>
                    {
                        new() { ResourceId = "string" }
                    }
                },
                new UserPartyContactInfo
                {
                    UserId = UserId,
                    PartyUuid = partyGuid2,
            ...
            var notificationAddresses = JsonSerializer.Deserialize<List<NotificationSettingsResponse>>(
                responseContent, _serializerOptionsCamelCase);
        
            Assert.Equal(2, notificationAddresses.Count);
            Assert.Contains(notificationAddresses, n => n.EmailAddress == "inbox@me.com");
            Assert.Contains(notificationAddresses, n => n.EmailAddress == "mail@address.com");
            Assert.All(notificationAddresses, n => Assert.NotNull(n.ResourceIncludeList));
        }
