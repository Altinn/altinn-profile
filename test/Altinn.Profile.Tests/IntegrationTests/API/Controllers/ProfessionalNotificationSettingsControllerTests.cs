...
namespace Altinn.Profile.Tests.IntegrationTests.API.Controllers
{
    public class ProfessionalNotificationSettingsControllerTests : IClassFixture<ProfileWebApplicationFactory<Program>>
    {
        ...
        [Fact]
        public async Task GetAllNotificationAddresses_WhenSystemUser_ReturnsForbidden()
        {
            HttpClient client = _factory.CreateClient();
        ...
        public async Task PutNotificationAddress_WhenContactInfoIsNew_ReturnsCreatedAndOrdersNotificationWithCountryCode(string resourceUrn, string sanitizedResourceId)
        {
            ...
            _factory.ProfessionalNotificationsRepositoryMock.Verify(
                ...
                Times.Once);
            _factory.AddressVerificationRepositoryMock.Verify(x => x.GetVerificationStatusAsync(It.IsAny<int>(), It.IsAny<AddressType>(), It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Exactly(2));
        }
        ...
        [Fact]
        public async Task PutNotificationAddress_WhenAddressUpsertReturnsFalse_ReturnsNoContent()
        {
            // Arrange
            const int UserId = 2516356;
            var partyGuid = Guid.NewGuid();
